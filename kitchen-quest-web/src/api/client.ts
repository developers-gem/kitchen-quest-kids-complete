...
// }
import { getAccessToken, refreshAccessToken, setAccessToken } from "./tokenStore";
import { getGateToken, setGateToken } from "./gateTokenStore";
import type { ApiErrorEnvelope, ApiSuccessEnvelope } from "../types/api";

// Fallback to "/api/v1" if VITE_API_BASE_URL is missing or lacks the version prefix
...
/**
 * Ensures the target URL always carries the "/api/v1" prefix and properly
 * formats search parameters without losing route prefixes.
 */
function buildUrl(path: string, query?: RequestOptions["query"]): string {
  // Strip origin if a full URL was provided in environment variables
  let basePath = RAW_BASE_URL.replace(/^https?:\/\/[^/]+/, "").replace(/\/+$/, "");

  // Guarantee /api/v1 prefix exists
  if (!basePath.startsWith("/api/v1")) {
    basePath = `/api/v1${basePath ? (basePath.startsWith("/") ? basePath : `/${basePath}`) : ""}`;
  }

  const cleanPath = path.startsWith("/") ? path : `/${path}`;
  const finalPath = cleanPath.startsWith(basePath) ? cleanPath : `${basePath}${cleanPath}`;
...
export async function requestPaginated<T>(
  ...
  return {
    data,
    meta: (meta as { page: number; limit: number; total: number; totalPages: number }) ?? {
      page: 1,
      limit: 0,
      total: 0,
      totalPages: 1,
...
// Single-flight lock to deduplicate concurrent refresh calls
...
async function requestEnvelope<T>(
  ...
  // Never attempt a silent refresh on auth endpoints to prevent loops
  ...
  if (res.status === 401 && !isAuthEndpoint && !options._isRetry) {
    ...
    // Refresh completely failed; reset token stores to trigger logged-out UI state
    setAccessToken(null);
    if (typeof setGateToken === "function") {
      setGateToken(null);
    }
  }
